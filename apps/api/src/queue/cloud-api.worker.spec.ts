...
describe('CloudApiWorker', () => {
  ...
  describe('media attachment send path', () => {
    ...
    it('does NOT mark the message FAILED when more attempts remain — leaves it retryable', async () => {
      mockCloudApi.sendTemplate.mockRejectedValueOnce(new Error('transient network error'));
    ...
    it('marks the message FAILED on the final configured attempt', async () => {
      mockCloudApi.sendTemplate.mockRejectedValueOnce(new Error('still failing'));
